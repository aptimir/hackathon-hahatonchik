"""Основной конвертер PDF в Markdown на базе Docling.

Скрипт обрабатывает набор PDF-документов, извлекает из них Markdown двумя
способами - с OCR и без OCR, а затем выполняет постобработку результата и
выбирает лучший вариант по набору эвристик качества.

Ключевые задачи скрипта:
- корректно сохранить Markdown-файлы
- привести имена изображений к требуемому формату
- убрать шум, пустые таблицы и битые ссылки на изображения
- выбрать лучший кандидат между OCR и no-OCR режимами.
"""
...
def _doc_num_from_stem(stem: str) -> int:
    """Извлекает номер документа из имени `document_NNN`.
...
def _choose_best_markdown(
    ...
    """Выбирает лучший Markdown-кандидат между OCR и no-OCR режимами."""
    candidates: list[tuple[float, str, str]] = []

    for mode_name, converter in (("no_ocr", no_ocr_converter), ("ocr", ocr_converter)):
        try:
            markdown = _extract_single_markdown(pdf_path, output_dir, converter, mode_name)
            quality = QualityStats(markdown)
            candidates.append((quality.score(), mode_name, markdown))
        except Exception:
            continue

    if not candidates:
        raise RuntimeError(f"Both OCR and no-OCR parsing failed for {pdf_path.name}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Baseline+ v5.5 (Docling): PDF -> Markdown")
